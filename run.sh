...
if [ ! -n "$WERCKER_EMAIL_NOTIFY_FAILED_SUBJECT" ]; then
  if [ ! -n "$DEPLOY" ]; then
    ...
    export WERCKER_EMAIL_NOTIFY_FAILED_SUBJECT="[wercker][$WERCKER_APPLICATION_NAME] Deploy Failed - $WERCKER_GIT_BRANCH"
  fi
fi
...
if [ ! -n "$WERCKER_EMAIL_NOTIFY_PASSED_SUBJECT" ]; then
  if [ ! -n "$DEPLOY" ]; then
    ...
    export WERCKER_EMAIL_NOTIFY_PASSED_SUBJECT="[wercker][$WERCKER_APPLICATION_NAME] Deploy Passed - $WERCKER_GIT_BRANCH"
  fi
fi
